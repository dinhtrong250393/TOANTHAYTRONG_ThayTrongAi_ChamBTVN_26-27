...
const original = fs.readFileSync('src/components/TeacherTextbookTab.tsx', 'utf8');

const matchToken = '        {currentLesson && (';
const index = original.indexOf(matchToken);
if (index !== -1) {
  const matchTokenEnd = '        )}';
  const subContent = original.substring(index);
  const endIndex = index + subContent.indexOf(matchTokenEnd) + matchTokenEnd.length;
  
  const finalContent = original.substring(0, endIndex) + '\n' + part1;
  fs.writeFileSync('src/components/TeacherTextbookTab.tsx', finalContent);
  console.log("Success");
} else {
  console.log("Failed to match token");
}
