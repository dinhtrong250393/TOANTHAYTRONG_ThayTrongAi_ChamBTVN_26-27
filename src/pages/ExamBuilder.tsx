...
import { db, handleFirestoreError, OperationType, syncTeacherSummary, syncExamsCover, syncExamResultsCover, uploadBase64ToStorage } from '../lib/firebase';
import { collection, addDoc, doc, getDoc, updateDoc, getDocs, query, where } from 'firebase/firestore';
import { useNavigate, useParams } from 'react-router-dom';
import { GoogleGenAI, Type } from '@google/genai';
import { Upload, Loader2, Save, ArrowLeft, Image as ImageIcon, Check, FileText, Edit2, Trash2 } from 'lucide-react';
import { Link } from 'react-router-dom';
import 'katex/dist/katex.min.css';
import MathText from '../components/MathText';
import { syncClassSummary, saveExamMetadata } from '../lib/syncUtils';
...
export default function ExamBuilder() {
      ...
      const fetchExam = async () => {
        try {
          ...
          if (docSnap.exists()) {
            ...
            setOriginalClasses(data.assignedClasses || []);
  ...
  const handleSaveExam = async (status: 'draft' | 'published') => {
    ...
    try {
      // Process questions to upload any base64 images to Storage
      const processedQuestions = await Promise.all(
        questions.map(async (q: any, qIdx: number) => {
          const newQ = { ...q };
          if (newQ.imageUrl && newQ.imageUrl.startsWith('data:')) {
            newQ.imageUrl = await uploadBase64ToStorage(
              newQ.imageUrl,
              `exam_questions/${appUser?.uid || 'anon'}_${Date.now()}_q${qIdx}_main.jpg`
            );
          }
          if (Array.isArray(newQ.imageUrls) && newQ.imageUrls.length > 0) {
            newQ.imageUrls = await Promise.all(
              newQ.imageUrls.map(async (img: string, iIdx: number) => {
                if (img && img.startsWith('data:')) {
                  return await uploadBase64ToStorage(
                    img,
                    `exam_questions/${appUser?.uid || 'anon'}_${Date.now()}_q${qIdx}_img${iIdx}.jpg`
                  );
                }
                return img;
              })
          ...
          if (Array.isArray(newQ.explanationImageUrls) && newQ.explanationImageUrls.length > 0) {
            newQ.explanationImageUrls = await Promise.all(
              newQ.explanationImageUrls.map(async (img: string, eIdx: number) => {
                if (img && img.startsWith('data:')) {
                  return await uploadBase64ToStorage(
                    img,
                    `exam_explanations/${appUser?.uid || 'anon'}_${Date.now()}_q${qIdx}_exp${eIdx}.jpg`
                  );
                }
                return img;
              })
      ...
      if (examId) {
        ...
        await saveExamMetadata(examId, updatedData);
        ...
        await saveExamMetadata(docRef.id, examData);
        
        if (appUser?.uid) {
          await syncExamResultsCover(docRef.id);
        }
      }
      
      const allAffectedClasses = Array.from(new Set([...originalClasses, ...classesArray]));
      ...
      if (appUser?.uid) {
        ...
        if (examId) {
          await syncExamResultsCover(examId);
        } else {
