...
export default function Login() {
  ...
  const handleStudentRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const targetClassName = regForm.classSelected || regForm.schoolInfo || '';
      const newPendingStudentData = {
      ...
      const docRef = await addDoc(collection(db, 'pending_students'), newPendingStudentData);

      if (targetClassName) {
        try {
          const newPendingStudentObj = {
            id: docRef.id,
            ...newPendingStudentData
          };
          const coverRef = doc(db, 'class_students_cover', targetClassName);
          const coverSnap = await getDoc(coverRef).catch(() => null);
          let existingStudents: any[] = [];
          if (coverSnap && coverSnap.exists()) {
            existingStudents = coverSnap.data().students || [];
          }
          // Remove duplicate if email already exists
          existingStudents = existingStudents.filter((s: any) => s.email !== generatedEmail && s.id !== docRef.id);
          existingStudents.push(newPendingStudentObj);

          // Sort list alphabetically by Vietnamese name
          existingStudents.sort((a: any, b: any) => {
            const nameA = a.name || '';
            const nameB = b.name || '';
          ...
          console.warn("Could not update class_students_cover on registration:", coverErr);
        }
      }
