...
export default function StudentDashboard() {
  ...
  const sortItemsByTitleNumber = (items: any[]) => {
  ...
  const fetchClassSummary = async (forceDirect = false) => {
    if (!appUser?.className || (hasFetchedSummaryRef.current && !forceDirect)) return;
    setIsRefreshing(true);

    const cacheKey = `student_summary_${appUser.className}`;
    const cacheTimeKey = `student_summary_time_${appUser.className}`;
    ...
    if (!forceDirect) {
      try {
        ...
        if (cachedStr && cachedTimeStr) {
          ...
          if (age < 10 * 1000) { // 10 seconds cache to prevent F5 spam but allow fresh data
            const cachedData = JSON.parse(cachedStr);
            setKnowledges(cachedData.knowledges || []);
            setExams(cachedData.exams || []);
            setEssays(cachedData.essays || []);
        ...
        console.warn("Error reading student summary cache:", e);
      }
    }
                ...
                <div className="bg-white shadow-sm overflow-hidden sm:rounded-[2rem] border border-slate-200/60 p-2">
                  <ul className="divide-y divide-gray-100">
                    {sortedEssays.map((essay) => {
                      ...
                      const isAvailable = (!essay.startTime || new Date(essay.startTime) <= new Date()) && !isEnded;
                                ...
                                <div className="flex flex-col items-end">
                                  <div className={`flex items-center font-semibold text-sm mb-2 px-3 py-1 rounded-full ${
                                    ...
                                    {isGraded ? (
                                      ...
                                      <Loader2 className="w-4 h-4 mr-1.5 text-amber-500" />
                                    )}
                                    {isGraded 
                                      ? `Đã chấm điểm (${submissionInfo.score?.toFixed(1) || '0.0'}/10)` 
                                      : isKeyError 
                                        ? 'Lỗi API Key (Bấm vào sửa)' 
                                        : (submissionInfo.status === 'grading' ? 'Đang tự động chấm...' : 'Chờ giáo viên chấm')}
                                  </div>
                                  <Link
