...
import { db, handleFirestoreError, OperationType, syncExamResultsCover } from '../lib/firebase';
...
export default function TakeExam() {
  ...
  useEffect(() => {
    const fetchExam = async () => {
      ...
      try {
        setError(null);
        // Check sessionStorage cache first to prevent repeated Firestore reads on F5 refresh
        const cachedExam = sessionStorage.getItem('cached_exam_' + examId);
        if (cachedExam) {
          try {
            ...
            sessionStorage.removeItem('cached_exam_' + examId);
          }
        }
        ...
        if (docSnap.exists()) {
          ...
          try {
            sessionStorage.setItem('cached_exam_' + examId, JSON.stringify(data));
          } catch (e) {}
        } else {
  ...
  // Load draft answers and time left on mount/exam load
  useEffect(() => {
    if (!examId || !exam) return;
    
    // Load answers draft from localStorage
    const savedAnswers = localStorage.getItem(`draft_answers_${examId}`);
    if (savedAnswers) {
      try {
        setAnswers(JSON.parse(savedAnswers));
      } catch (e) {
        console.error("Error parsing saved answers draft", e);
      }
    }

    // Load timeLeft draft from localStorage to prevent cheating or reset
    const savedTimeLeft = localStorage.getItem(`draft_timeleft_${examId}`);
    if (savedTimeLeft) {
      const parsedTime = Number(savedTimeLeft);
      if (!isNaN(parsedTime) && parsedTime > 0) {
        // Only override if the new saved time is less than the current time left
        setTimeLeft(prev => {
          if (prev === null) return parsedTime;
          return Math.min(prev, parsedTime);
        });
      }
    }
  }, [examId, exam]);
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(() => {
      if (Object.keys(answers).length > 0) {
        localStorage.setItem(`draft_answers_${examId}`, JSON.stringify(answers));
      }
    }, 1000);
    return () => clearTimeout(handler);
  }, [answers, examId, exam]);

  // Save timeLeft draft to localStorage periodically or on change
  useEffect(() => {
    if (!examId || !exam || timeLeft === null) return;
    localStorage.setItem(`draft_timeleft_${examId}`, timeLeft.toString());
  }, [timeLeft, examId, exam]);

  useEffect(() => {
    if (timeLeft === null || timeLeft <= 0 || submittedResult || isSubmitting) {
      if (timeLeft === 0 && !isSubmitting && !submittedResult) {
        handleSubmit();
      }
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await setDoc(examRef, { submissionSummary: arrayUnion(summaryItem) }, { merge: true }).catch(e => console.warn("exams cover update failed:", e));

      const examMetaRef = doc(db, 'exams_metadata', exam.id);
      await setDoc(examMetaRef, { submissionSummary: arrayUnion(summaryItem) }, { merge: true }).catch(e => console.warn("exams_metadata update failed:", e));

      // Update the user document so the student can see their result without fetching all exams
      const userRef = doc(db, 'users', appUser.uid);
      await setDoc(userRef, { completedExams: arrayUnion({ examId: exam.id, submissionId: docRef.id, score, submittedAt: submissionData.submittedAt }) }, { merge: true }).catch(e => console.warn("user exams summary update failed:", e));
      const coverRef = doc(db, 'exam_results_cover', exam.id);
      await setDoc(coverRef, {
        submissionSummary: arrayUnion(summaryItem)
      }, { merge: true }).catch(e => console.warn("exam_results_cover update failed:", e));

      setSubmittedResult({ score, incorrectQuestions });
      try {
        localStorage.removeItem(`draft_answers_${exam.id}`);
        localStorage.removeItem(`draft_timeleft_${exam.id}`);
      } catch (e) {}
