  ...
  const handleGenerateBarem = async (index: number) => {
    ...
    try {
      ...
      if (!aiRes.ok) {
        ...
        throw new Error(errorData.error || 'Lỗi kết nối đến AI');
      }
      const aiData = await aiRes.json();
      
      const finalSolution = aiData.solutionText || aiData.solution || aiData.barem || aiData.answer || '';
      
      updateItem(index, {
        title: aiData.title || item.title,
        questionText: aiData.questionText || aiData.question || '',
      ...
      if (!finalSolution) {
         updateItem(index, { errorMessage: "AI không trả về kết quả lời giải. Bạn hãy thử nhấn lại nút Tạo lại bằng AI." });
      } else {
         updateItem(index, { errorMessage: "" });
      }
      ...
      updateItem(index, { 
        isAnalyzing: false, 
        errorMessage: err.message || 'Có lỗi xảy ra khi kết nối AI.'
      });
    }
  };
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center">
            <button onClick={() => navigate(-1)} className="mr-4 p-2 -ml-2 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-100 transition-colors">
                    ...
                    <div className="space-y-6">
                      ...
                      {item.errorMessage && (
                         <div className="bg-red-50 border border-red-200 rounded-2xl p-4 text-center mb-4">
                            <p className="text-sm font-bold text-red-600 mb-1">Lỗi Xảy Ra</p>
                            <p className="text-sm text-red-500">{item.errorMessage}</p>
                            <button
                              onClick={() => handleGenerateBarem(index)}
                              className="mt-3 bg-red-100 text-red-700 px-4 py-2 rounded-lg font-bold text-xs hover:bg-red-200 transition-colors"
                            >
                              Thử Lại Ngay
                            </button>
                         </div>
